...
pub enum Sound {
    ...
    SND_MISSILEBOOM = 9,
    SND_ACIDSPLAT = 10,
    /// little red bullets hitting the wall
    SND_ENERGYBONK = 11,
    SND_BOMBBOOM = 12,
    SND_BULLETHIT = 13,
    SND_MEGABEAMHIT = 14,
    ...
    /// each piece of flame being thrown does this
    SND_FLAMEGO = 17,
    /// machine gun (1 shot)
    SND_BULLETFIRE = 18,
    /// when you get all the brains
    SND_KOOLKAT = 19,
    /// being fired
    SND_MEGABEAM = 20,
    SND_HAMMERTOSS = 21,
    /// skip level via cheat code
    SND_CHEATWIN = 22,
    SND_HEALTHCHEAT = 23,
    /// doesn't exist yet, but someday
    SND_INVINCCHEAT = 24,
    SND_DROWN = 25,
    SND_ITEMDROP = 26,
    SND_GOTOMAP = 27,
    ...
    SND_ZOMBIEDIE = 49,
    SND_ZOMBIELEAP = 50,
    /// not implemented yet, but random moans from the zombies while they wander
    SND_ZOMBIEMOAN = 51,
    // egg sac
    SND_EGGSACDIE = 52,
    SND_EGGSACBIRTH = 53,
    ...
    /// this is when her head or skull dies
    SND_MATTIEDIE = 69,
    SND_MATTIECLAW = 70,
    SND_MATTIESHOOT = 71,
    SND_MATTIESTOMP = 72,
    ...
    SND_GINGERCOMBO = 76,
    // pumpkin
    /// no ouch sound- they usually die in one hit, if not who cares
    SND_PUMPKINDIE = 77,
    ...
    /// rocks bouncing off wall, not really roly poly, but kind of
    SND_ROCKBOUNCE = 131,
    // sphinx
...
#[no_mangle]
pub unsafe extern fn MakeSound(snd: Sound, mut x: c_int, mut y: c_int, flags: SoundFlags, priority: c_int) {
