...
void AI_GoodTurret(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(me->action==ACTION_BUSY)
	{
		...
		{
			FireBullet(me->x-FIXAMT*30+MGL_randoml(FIXAMT*60),me->y-FIXAMT*22+MGL_randoml(FIXAMT*44),
...
void AI_Mattie2Brain(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(me->ouch==4 && me->type==MONS_MAT2BRAIN)	// skull and head have their own ouch noises
	{
		if(me->hp>0)
	...
	if(me->action!=ACTION_BUSY)
	{
		...
		if(me->type!=MONS_MAT2BRAIN && me->facing!=0)
			me->facing+=4;
	}
	else
...
void AI_Mattie2Tail(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(me->action==ACTION_BUSY)
	{
		if(me->seq==ANIM_ATTACK && me->reload==0)
		{
			...
			if(me->parent->type==MONS_MAT2BRAIN)
			{
				b+=20-MGL_random(41);
				FireExactBullet(x,y,me->z+32*FIXAMT,Cosine(b)*10,Sine(b)*10,FIXAMT*12,
...
void AI_BuddyBunny(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(me->action==ACTION_BUSY)
	{
		...
		{
			for(x=0;x<8;x++)
				SpecialSnow(me->x+((-16+MGL_random(33))<<FIXSHIFT),
							me->y+((-16+MGL_random(33))<<FIXSHIFT));
		}
		return;
	}
	...
	{
		me->mind1=0;
		if(me->type==MONS_FRIENDLY)
			me->mind3=MGL_random(4)*64;
...
void AI_PygmyQueen(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(me->action==ACTION_BUSY)
	{
		if(me->seq==ANIM_ATTACK && me->frm==2 && me->reload==0 && goodguy)
		{
			...
			for(i=0;i<5;i++)
			{
				x=me->x-200*FIXAMT+MGL_randoml(400*FIXAMT);
				y=me->y-170*FIXAMT+MGL_randoml(340*FIXAMT);
				g=AddGuy(x,y+FIXAMT*40,FIXAMT*20,MONS_MINIPYGMY);
	...
	else
	{
		...
		if(RangeToTarget(me,goodguy)<(500*FIXAMT) && MGL_random(64)==0 && me->reload==0)
		{
			// make ultrapygmies!
...
void AI_Jalapeno(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(me->mind3==0)
	{
		BlowSmoke(me->x,me->y,FIXAMT*10,MGL_random(4)*FIXAMT);
		me->mind3=(byte)MGL_random(10);
	}
	else
		me->mind3--;
...
void AI_Generator(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	else
	{
		...
		{
			// summon!
			g=AddGuy(me->x,me->y,10*FIXAMT,me->mind);
			if(g && !g->CanWalk(g->x,g->y,map,world))
			{
				g->type=MONS_NONE;
			}
			else
			{
				...
				for(x=0;x<15;x++)
					BlowSmoke(me->x-40*FIXAMT+MGL_randoml(80*FIXAMT),me->y-30*FIXAMT+MGL_randoml(60*FIXAMT),
						0,MGL_random(8)*FIXAMT+FIXAMT);
...
void AI_Shark(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(me->action==ACTION_BUSY)
	{
		...
		{
			FaceGoodguy(me,goodguy);
			FireBulletZ(me->x,me->y,FIXAMT*4,me->facing*32-16+(byte)MGL_random(32),BLT_SHARK,me->friendly);
		}
		return;
	}
	...
	if(me->mind==0)	// hiding
	{
		...
		{
			me->mind=1;
			me->mind1=40+(byte)MGL_random(60);
...
void AI_Wizard(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(goodguy)
	{
		...
		if(RangeToTarget(me,goodguy)<(500*FIXAMT) && MGL_random(50)==0 && me->reload==0)
		{
			// shoot a shroom
			me->seq=ANIM_A1;
			me->frm=0;
...
void AI_Bob(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	{
		if(RangeToTarget(me,goodguy)<(1024*FIXAMT) && MGL_random(256)==0)
		{
			// yell for help
			MakeSound(SND_GINGERJKICK,me->x,me->y,SND_CUTOFF,1200);
			me->seq=ANIM_A3;
...
void AI_MultiMoss(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(me->action==ACTION_BUSY)
	{
		if(me->seq==ANIM_DIE && me->frm==2 && me->reload==0)
		{
			// divide in two!
			me->reload=100;
			x=me->x-FIXAMT*48+MGL_randoml(FIXAMT*96);
			y=me->y-FIXAMT*32+MGL_randoml(FIXAMT*64);
			g=AddGuy(x,y,0,MONS_MULTIMOSS);
			if(g && !g->CanWalk(g->x,g->y,map,world))
				g->type=MONS_NONE;
			else if(g)
			{
		...
		switch(MGL_random(4))
		{
			case 0:
				...
				if(x>0 && map->map[x-1+y*map->width].wall==0 &&
					...
					&& (!MossCheck(x-1,y)))
				{
					baby=AddGuy(me->x,me->y,0,MONS_MOSS2);
					if(baby)
					{
...
void AI_Snowball(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(me->action==ACTION_BUSY)
	{
		if(me->seq==ANIM_DIE)
		{
			...
			BlowSmoke(me->x+((32-MGL_random(65))<<FIXSHIFT),me->y+((32-MGL_random(65))<<FIXSHIFT),
				MGL_random(48)*FIXAMT,MGL_random(5)*FIXAMT);
			BlowSmoke(me->x+((32-MGL_random(65))<<FIXSHIFT),me->y+((32-MGL_random(65))<<FIXSHIFT),
				MGL_random(48)*FIXAMT,MGL_random(5)*FIXAMT);
			BlowSmoke(me->x+((32-MGL_random(65))<<FIXSHIFT),me->y+((32-MGL_random(65))<<FIXSHIFT),
				MGL_random(48)*FIXAMT,MGL_random(5)*FIXAMT);
			FireBullet(me->x+((32-MGL_random(65))<<FIXSHIFT),me->y+((32-MGL_random(65))<<FIXSHIFT),
				(byte)MGL_random(256),BLT_BIGSNOW,me->friendly);
			FireBullet(me->x+((32-MGL_random(65))<<FIXSHIFT),me->y+((32-MGL_random(65))<<FIXSHIFT),
				(byte)MGL_random(256),BLT_BIGSNOW,me->friendly);
		}
		return;
...
void AI_XenoMama(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(me->mind==0)		// when mind=0, singlemindedly lumber towards Bouapha
	{
		if(MGL_random(32)==0 && RangeToTarget(me,goodguy)<800*FIXAMT && me->mind3==0)
		{
			// get him!
			me->mind3=60;
...
void AI_Gnome(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(me->action==ACTION_BUSY)
	{
		if(me->seq==ANIM_ATTACK && me->frm==6 && me->reload==0 && goodguy)
		{
			...
			{
				if(goodguy->type==MONS_BOUAPHA && me->mind3==0)
				{
	...
	else
	{
		switch(MGL_random(4))
		{
			...
			case 2:	// right of the world
				...
				me->y=(MGL_random(map->height*2)-map->height/2)*TILE_HEIGHT*FIXAMT;
				me->mind=MGL_random(60)+2;
				break;
...
void AI_BigPumpkin(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(me->action==ACTION_BUSY)
	{
		if(me->seq==ANIM_DIE && me->reload==0)
		{
			...
			me->reload=2;
			x=me->x-FIXAMT*32+MGL_randoml(FIXAMT*64);
			y=me->y-FIXAMT*24+MGL_randoml(FIXAMT*48);
			g=AddGuy(x,y,0,MONS_PUMPKIN);
			if(g && !g->CanWalk(g->x,g->y,map,world))
				g->type=MONS_NONE;
		}
		return;	// can't do nothin' right now
...
void AI_CrazyBone(Guy *me,Map *map,world_t *world,Guy *goodguy)
{
	...
	if(me->action==ACTION_BUSY)
	{
		...
		if(me->seq==ANIM_DIE && me->frm==7 && MGL_random(6)==0)
		{
			// come back!
			me->seq=ANIM_A3;
