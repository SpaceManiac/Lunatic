void AI_MineCart(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	if (me->mind == 0) // waiting to be ridden
	{
		...
		if (me->mapx < map->width - 1 && world->terrain[map->map[me->mapx + 1 + me->mapy * map->width].floor].flags & TF_MINECART)
			ok[0] = 1;
		else
			ok[0] = 0;
		if (me->mapy < map->height - 1 && world->terrain[map->map[me->mapx + (me->mapy + 1) * map->width].floor].flags & TF_MINECART)
			ok[1] = 1;
		else
			ok[1] = 0;
		if (me->mapx > 0 && world->terrain[map->map[me->mapx - 1 + me->mapy * map->width].floor].flags & TF_MINECART)
			ok[2] = 1;
		else
			ok[2] = 0;
		if (me->mapy > 0 && world->terrain[map->map[me->mapx + (me->mapy - 1) * map->width].floor].flags & TF_MINECART)
			ok[3] = 1;
		else
			ok[3] = 0;
...
sorryimusingagoto:
			...
			// solid object, bounce back
			if (x < 0 || x >= map->width || y < 0 || y >= map->height ||
					map->map[x + y * map->width].wall > 0 ||
					(map->map[x + y * map->width].item >= MAX_WALKABLE_ITMS &&
					map->map[x + y * map->width].item < NEW_PICKUP_ITMS))
			{
				me->x -= me->dx;
				me->y -= me->dy;
				me->facing = (me->facing + 2)&3;
				goto sorryimusingagoto;
			}
				// otherwise if it's not water, we're done
			else if (!(world->terrain[map->map[x + y * map->width].floor].flags & (TF_WATER | TF_LAVA)))
			{
...
void AI_Vampire(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	if (me->type == MONS_DARKVAMP)
	{
		// get burned by the light!
		if (map->map[me->mapx + me->mapy * map->width].light > 0 && me->ouch == 0 && me->hp > 0)
		{
			monsType[me->type].flags = 0;
			me->GetShot(0, 0, map->map[me->mapx + me->mapy * map->width].light, map, world);
			monsType[me->type].flags = MF_INVINCIBLE;
			BlowSmoke(me->x, me->y, FIXAMT * 10, MGL_random(6) * FIXAMT);
			BlowSmoke(me->x, me->y, FIXAMT * 10, MGL_random(6) * FIXAMT);
			BlowSmoke(me->x, me->y, FIXAMT * 10, MGL_random(6) * FIXAMT);
			BlowSmoke(me->x, me->y, FIXAMT * 10, MGL_random(6) * FIXAMT);
			BlowSmoke(me->x, me->y, FIXAMT * 10, MGL_random(6) * FIXAMT);
...
void AI_Coffin(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	if (me->action == ACTION_BUSY)
	{
		...
		{
			AddGuy(me->x + FIXAMT * 20, me->y + FIXAMT * 10, 0, MONS_VAMPIRE);
			// then die
			me->hp = 0;
			me->seq = ANIM_DIE;
			me->frm = 0;
...
void AI_Ghost(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	if (me->action == ACTION_BUSY)
	{
		...
		if (me->seq == ANIM_A1 && me->frm == 2 && me->frmTimer > 64)
		{
			...
			me->x += -FIXAMT * 320 + MGL_randoml(FIXAMT * 640);
			me->y += -FIXAMT * 240 + MGL_randoml(FIXAMT * 480);
	...
	else if (me->mind == 1) // has spotted him, give chase
	{
		...
		if (goodguy)
		{
			...
			if (RangeToTarget(me, goodguy) < 140 * FIXAMT && MGL_random(20) == 1)
			{
				// scream
		...
		if (me->mind1)
		{
			...
			switch (me->mind2) {
				case 0:
					...
					break;
				case 1:
					if (!map->map[me->mapx + 0 + (me->mapy + 1) * map->width].wall)
						FireBullet(me->x, me->y + FIXAMT * TILE_HEIGHT / 2, 2, BLT_FLAME2, me->friendly);
					break;
				case 2:
					if (!map->map[me->mapx - 1 + (me->mapy + 0) * map->width].wall)
						FireBullet(me->x - FIXAMT * TILE_WIDTH / 2, me->y, 4, BLT_FLAME2, me->friendly);
					break;
				case 3:
					if (!map->map[me->mapx + (me->mapy - 1) * map->width].wall)
...
void AI_Pygmy2(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	else
	{
		...
		if (RangeToTarget(me, goodguy)<(512 * FIXAMT) && MGL_random(64) == 0 && me->reload == 0)
		{
			// spear him!
...
void AI_Pygmy3(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	else
	{
		...
		if (RangeToTarget(me, goodguy)<(200 * FIXAMT) && MGL_random(64) == 0 && me->reload == 0)
		{
			// zap him!
...
void AI_Pumpkinstein(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	if (me->mind == 0)
	{
		...
		{
			goodguy->GetShot(-4 * FIXAMT + MGL_randoml(8 * FIXAMT), -4 * FIXAMT + MGL_randoml(8 * FIXAMT), 10, map, world);
			me->mind1 = 1;
		}
		if (me->mind1)
		{
			...
			if (MGL_random(2) == 0)
			{
				me->mind2 = (me->facing + 4 + MGL_random(3))&7;
				me->mind3 = 0;
			}
			else
			{
...
void AI_Knight(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	if (me->ouch == 4)
	{
		if (me->hp > 0)
		{
			if (me->action != ACTION_BUSY && MGL_random(2) == 0)
			{
				me->action = ACTION_BUSY;
	...
	else
	{
		if (goodguy)
		{
			...
			if (RangeToTarget(me, goodguy) < 128 * FIXAMT && goodguy->y > me->y && MGL_random(10) == 0)
			{
				MakeSound(SND_SKELKICK, me->x, me->y, SND_CUTOFF, 1200);
				me->dx = 0;
				me->dy = 0;
...
void AI_Triceroid(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	x = monsType[me->type].hp - me->hp + 5; // more damaged=faster
...
void AI_Countess(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	if (me->action == ACTION_BUSY)
	{
		...
		{
			me->reload = 2;
			g = AddGuy(me->x, me->y + FIXAMT * 40, FIXAMT * 20, MONS_BAT);
			...
			else if (g)
			{
				...
				g->dx = Cosine(g->facing * 32)*12 - FIXAMT * 2 + MGL_randoml(FIXAMT * 4);
		...
		if (me->seq == ANIM_A1 && me->frm == 2 && me->reload == 0)
		{
			...
			for (i = 0; i < 3; i++)
			{
				g = AddGuy(me->x + xpos[i], me->y + ypos[i], 0, MONS_BONEHEAD);
				if (g && (!g->CanWalk(g->x, g->y, map, world)))
					g->type = MONS_NONE; // vanish if this spot is occupied
				else if (g)
				{
		...
		if (me->seq == ANIM_DIE && me->frm > 3)
		{
			...
			BlowSmoke((x + me->rectx + MGL_random(me->rectx2 - me->rectx)) << FIXSHIFT,
					(y + me->recty + MGL_random(me->recty2 - me->recty)) << FIXSHIFT,
					me->z - MGL_randoml(FIXAMT * 60), FIXAMT);
		}
	...
	if (me->mind == 1) // wandering mode
	{
		...
		if (RangeToTarget(me, goodguy) > 400 * FIXAMT && MGL_random(10) == 1)
		{
			MakeSound(SND_COUNTCHARGE, me->x, me->y, SND_CUTOFF, 1200);
			me->mind = 2;
			me->mind1 = 10 + MGL_random(10) + 1;
			FaceGoodguy(me, goodguy);
			me->facing = ((me->facing * 32) - 16 + MGL_random(33));
			me->action = ACTION_BUSY;
			me->seq = ANIM_A2;
			me->frm = 0;
			me->frmTimer = 0;
...
void AI_AlienEgg(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	if (me->action == ACTION_BUSY)
	{
		...
		{
			me->reload = 90;
			if (me->type == MONS_ALIENEGG2)
				me->reload = 10;
			g = AddGuy(me->x, me->y + FIXAMT * 40, FIXAMT * 20, MONS_BABYALIEN);
			if (g && (!g->CanWalk(g->x, g->y, map, world)))
				g->type = MONS_NONE; // vanish if this spot is occupied
			else if (g)
			{
				g->facing = (byte) MGL_random(8);
		...
		if (me->seq == ANIM_DIE)
		{
			FireBullet(me->x, me->y, (byte) MGL_random(256), BLT_ACID, me->friendly);
	...
	{
		if (me->reload == 0 && (MGL_random(150) == 0 || me->type == MONS_ALIENEGG2))
		{
...
void AI_BabyAlien(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	if (me->mind1)
	{
		...
		if (RangeToTarget(me, goodguy) < 160 * FIXAMT && MGL_random(20) == 0)
		{
...
void AI_Alien(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	if (me->mind == 0) // when mind=0, singlemindedly lumber towards Bouapha
	{
		if (goodguy)
		{
			if (RangeToTarget(me, goodguy)<(78 * FIXAMT) && MGL_random(8) == 0)
			{
				// get him!
				MakeSound(SND_SKELKICK, me->x, me->y, SND_CUTOFF, 1200);
				me->seq = ANIM_ATTACK;
	...
	else if (me->mind == 1) // random wandering
	{
		if (goodguy)
		{
			if (RangeToTarget(me, goodguy)<(512 * FIXAMT) && MGL_random(16) == 0)
			{
				// spit at him
...
void AI_Robopumpkin(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	if (me->action == ACTION_BUSY)
	{
		...
		if (me->seq == ANIM_A1 && me->reload == 0 && goodguy)
		{
			...
			FireBulletZ(x, y, FIXAMT * 50, me->facing * 32 - 16 + MGL_random(33), BLT_ENERGY, me->friendly);
	...
	if (me->mind == 0) // when mind=0, singlemindedly lumber towards Bouapha
	{
		if (goodguy)
		{
			if (RangeToTarget(me, goodguy)<(96 * FIXAMT) && MGL_random(8) == 0)
			{
				// get him!
				me->seq = ANIM_ATTACK;
				me->frm = 0;
...
void AI_ShockTrooper(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	if (me->action == ACTION_BUSY)
	{
		...
		if (me->seq == ANIM_A1 && (me->frm == 3 || me->frm == 4) && me->reload == 0 && goodguy)
		{
			...
			FireBulletZ(x, y, FIXAMT * 100, me->facing * 32 - 16 + MGL_random(33), BLT_GRENADE, me->friendly);
			if (me->frm == 4)
				me->reload = 30;
		}
	...
	if (!goodguy)
		return;
	if (RangeToTarget(me, goodguy)<(300 * FIXAMT) && MGL_random(20) == 0)
	{
		// get him!
...
void AI_Minibot(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	else if (me->mind == 1) // random wandering
	{
		if (goodguy)
		{
			if (RangeToTarget(me, goodguy)<(512 * FIXAMT) && MGL_random(256) == 0 && me->reload == 0)
			{
				// shoot at him
				MakeSound(SND_ROBOSHOOT, me->x, me->y, SND_CUTOFF, 1200);
				FireBullet(me->x, me->y, me->facing * 32, BLT_ENERGY, me->friendly);
...
void AI_Meaniebot(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	if (me->action == ACTION_BUSY)
	{
		if (me->seq == ANIM_ATTACK && me->frm > 1 && me->frm < 5 && me->reload == 0)
		{
			...
			FireBullet(x, y, me->facing * 32 - 16 + MGL_random(33), BLT_ENERGY, me->friendly);
			x = me->x + Cosine(((me->facing - 2)&7)*32)*24;
			y = me->y + Sine(((me->facing - 2)&7)*32)*20;
			FireBullet(x, y, me->facing * 32 - 16 + MGL_random(33), BLT_ENERGY, me->friendly);
			me->reload = 2;
	...
	else if (me->mind == 1) // random wandering
	{
		if (goodguy)
		{
			if (RangeToTarget(me, goodguy)<(512 * FIXAMT) && MGL_random(256) == 0)
			{
				// shoot at him
				me->seq = ANIM_ATTACK;
				me->frm = 0;
...
void AI_Robofactory(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	if (me->action == ACTION_BUSY)
	{
		...
		{
			g = AddGuy(me->x, me->y, 0, MONS_ROBOT1);
			if (g && (!g->CanWalk(g->x, g->y, map, world)))
				g->type = MONS_NONE; // vanish if this spot is occupied
			me->reload = 30;
			me->mind1 = 8;
...
void AI_Bunny(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	if (me->ouch == 4)
	{
		...
		for (x = 0; x < y; x++)
		{
			i = MGL_random(12);
			if (facingMode[i] == 0)
				FireBullet(me->x, me->y, (byte) MGL_random(8), bltTable[i], me->friendly);
			else
				FireBullet(me->x, me->y, (byte) MGL_random(256), bltTable[i], me->friendly);
		}
	}
...
void AI_LoonyBot(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	if (me->action == ACTION_BUSY)
	{
		...
		if (me->seq == ANIM_DIE)
		{
			...
			{
				AddGuy(me->x, me->y - FIXAMT * 96, 0, MONS_LOONYSHIP);
			}
		}
		return;
	}
	...
	else
	{
		if (me->seq != ANIM_MOVE)
		{
			...
			else
			{
				...
				FireBullet(me->x - 87 * FIXAMT, me->y - 30 * FIXAMT, MGL_random(88) + 20, BLT_GRENADE, me->friendly);
				FireBullet(me->x + 92 * FIXAMT, me->y - 30 * FIXAMT, MGL_random(88) + 20, BLT_GRENADE, me->friendly);
...
void AI_LoonyGun(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	switch (me->mind1) {
		...
		case 5:
			...
			diff = me->facing * 16 - 10 + MGL_random(20);
...
void AI_LoonyShip(Guy *me, Map *map, world_t *world, Guy *goodguy)
{
	...
	else
	{
		me->mind2 = 0;
		// turn randomly
		me->mind = (me->mind + 1 - MGL_random(3))&7;
	}
	...
	if (me->mind3)
	{
		if ((me->mind3 & 1) == 0)
		{
			x = me->x + Cosine(me->facing * 32)*40;
			y = me->y + Sine(me->facing * 32)*32;
			f = me->facing * 32 - 5 + MGL_random(10);
