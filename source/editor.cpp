...
byte InitEditor(void)
{
	...
	strcpy(MonsterName(MONS_ROLLER2), "Reverse Roly");
	strcpy(MonsterName(MONS_ROLLER4), "Reverse Tumbly");
...
void EnemyPickerClick(void)
{
	...
	for (i = 1; i < NUM_MONSTERS; i++)
	{
		...
		if (m != MONS_MATCLAW1 && m != MONS_MATCLAW2 && m != MONS_MATSKULL && m != MONS_MATBRAIN &&
				m != MONS_MATBODY && m != MONS_MATTAIL && m != MONS_THINGTENT && m != MONS_THINGTENTTIP &&
				m != MONS_SPHXARM1 && m != MONS_SPHXARM2 && m != MONS_CENTIBODY && m != MONS_PWRBOUAPHA &&
				m != MONS_LOONYGUN && m != MONS_LOONYCORE && m != MONS_MAT2SKULL && m != MONS_MAT2BRAIN &&
				m != MONS_MAT2BODY && m != MONS_MAT2TAIL && m != MONS_NOBODY)
		{
			if (mouseX > x - 1 && mouseX < x + 201 && mouseY > y - 1 && mouseY < y + 15)
			{
				editMode = EDITMODE_EDIT;
				editopt.curBadguy = (byte) m;
				return;
			}
		}
		x += 202;
	...
	if (mouseX > 0 && mouseX < 300 && mouseY > 466 && mouseY < 480)
		monsPage = 1 - monsPage;
}
...
void TerrainMouseClick(void)
{
	...
	if (n > 199) // clicked outside the display
	{
		...
		if (mouseX > 249 && mouseX < 249 + 12 && mouseY > 249 && mouseY < 249 + 12)
			world.terrain[editopt.curFloor].flags ^= TF_MINECART;
		if (mouseX > 249 && mouseX < 249 + 12 && mouseY > 261 && mouseY < 261 + 12)
			world.terrain[editopt.curFloor].flags ^= TF_BUNNY;
	}
	else
	{
...
void RenderTerrainInfo(byte which)
{
	...
	RenderCheckbox(250, 250, world.terrain[which].flags&TF_MINECART, "Mine-cart track");
	RenderCheckbox(250, 262, world.terrain[which].flags&TF_BUNNY, "Buddy Bunny Path");
...
void RenderEnemyPickDisplay(void)
{
	...
	for (i = 1; i < NUM_MONSTERS; i++)
	{
		...
		if (y > 450)
			break; // don't go off the page
	}
	if (mouseX > 0 && mouseX < 300 && mouseY > 466 && mouseY < 480)
		editmgl->Box(2, 466, 300, 479, 31);
	else
		editmgl->Box(2, 466, 300, 479, 16);

	Print(4, 468, "MORE MONSTERS!", 0, 1);
}
		...
		switch (k) {
			...
			case 'C':
				for (x = 0; x < curMap->width * curMap->height; x++)
					curMap->map[x].light = curMap->map[tileX + tileY * curMap->width].light;
				break;
			case '-':
				editopt.curFloor--;
			...
			case 'g':
				...
				for (x = 0; x < MAX_MAPMONS; x++)
				{
					if (curMap->badguy[x].x == tileX && curMap->badguy[x].y == tileY && curMap->badguy[x].type)
						editopt.curBadguy = curMap->badguy[x].type;
				}
				break;
		}
	}
